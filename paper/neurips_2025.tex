...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/nature_fig1_architecture_gpt.png}
    \caption{Architecture and design logic of SabreSDN. \textbf{a,} Complex-valued U-Net for mapping noisy complex spectra to denoised complex spectra. \textbf{b--d,} Complex convolution, CBAM attention, and gated skip connection mechanisms.}
    \label{fig:architecture}
\end{figure}

The architecture keeps the signal representation one-dimensional throughout the network, preserving the prior that a spectral feature should be processed similarly at any chemical shift. Because down-sampling and up-sampling are implemented with complex convolutions, the real and imaginary branches remain coupled at every resolution rather than being treated as unrelated feature maps.
...
The loss reflects the analytical priorities of low-concentration SABRE NMR: peak positions and multiplet substructure must be preserved, while the baseline must remain sufficiently flat for subsequent integration or peak picking. The explicit peak mask reduces the imbalance between the small chemically informative region and the much larger baseline region, while the baseline term discourages ringing outside the peaks.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\textwidth]{figures/nature_fig4_loss_deployment.pdf}
    \caption{Loss weighting and deployment constraints. \textbf{a,} Composite-loss weights. \textbf{b,} Peak-region emphasis. \textbf{c,} Inference latency relative to acquisition.}
    \label{fig:loss-deployment}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\textwidth]{figures/nature_fig2_training.pdf}
    \caption{Training dynamics and optimization summary. \textbf{a,} Training and validation loss. \textbf{b,} Learning-rate schedule. \textbf{c,} Rolling validation-loss trend. \textbf{d,} Dataset and optimization summary.}
    \label{fig:training}
\end{figure}
...
We evaluate SabreSDN using metrics that correspond to practical post-denoising tasks. The primary metric is the composite validation loss in Eq.~\eqref{eq:loss}, which combines whole-spectrum amplitude fidelity, peak-region reconstruction, and baseline smoothness. We additionally inspect peak-region absolute error over $\Omega_p=[6600,6800]$, off-peak baseline fluctuation, and inference latency per spectrum. These quantities are complementary: a spectrum with low peak error but high baseline fluctuation can still be unsuitable for automated integration, while an overly smooth spectrum with suppressed multiplet splitting would be chemically misleading.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\textwidth]{figures/nature_fig3_results.pdf}
    \caption{Representative denoising behavior on 50~$\mu$M SABRE-$^{19}$F test spectra. \textbf{a,} Held-out input and SabreSDN output in the peak region. \textbf{b,} Additional held-out examples. \textbf{c,} Off-peak RMS comparison. \textbf{d,} Recovered peak-region envelope.}
    \label{fig:denoising}
\end{figure}
...
The central result of this study is that low-concentration SABRE-$^{19}$F spectra can be denoised effectively by combining a physics-aligned complex representation with a U-Net-style restoration backbone. The gain does not arise from model size alone. Rather, complex convolutions constrain the real and imaginary channels to be processed as a coupled pair, CBAM1d attention emphasizes chemically occupied frequency regions, the dilated bottleneck captures line-shape context beyond a single kernel width, and gated skip connections reduce the risk that high-resolution noise is copied directly into the decoder output.

This design targets a regime in which conventional denoising is fragile. Exponential windowing broadens peaks, and Cadzow-style rank truncation depends on an unknown signal rank that can be unstable when weak peaks coexist with strong resonances. SabreSDN instead learns a nonlinear mapping while preserving the input grid and phase structure. Its millisecond-scale inference time also makes it suitable as a signal-recovery module within online SABRE workflows, although peak assignment, concentration quantification, uncertainty estimation, and kinetic modeling remain downstream tasks that require additional validation.
