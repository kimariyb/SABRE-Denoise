...
def inference(model, loader):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # plot the spectra
            plot_spectra(pred, label, save_path=None)

    return preds


def plot_spectra(pred, label, save_path=None):
    # 转换输入数据为numpy数组(如果是PyTorch张量)
...
if __name__ == "__main__":
    ...
    names = []
    for f in files:
        spectra = loadSpectra(os.path.join(INFER_DIR, f.name))
        # split data
